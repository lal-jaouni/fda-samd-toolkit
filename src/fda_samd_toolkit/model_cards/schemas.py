...
class ModelDetails(BaseModel):
    ...
    citation: Optional[str] = Field(
        None, description="BibTeX citation if model is published"
    )
    contact: Optional[str] = Field(None, description="Email or contact information for questions")
    release_date: Optional[date] = Field(None, description="Date model was released")
...
class Factors(BaseModel):
    ...
    datasets_used: List[str] = Field(
        default=[], description="Datasets used for evaluation (names/identifiers)"
    )
...
class MetricSubgroup(BaseModel):
    ...
    subgroup_definition: str = Field(
        ..., description="Definition of subgroup membership criteria"
    )
    ...
    confidence_intervals: Optional[Dict[str, tuple]] = Field(
        None,
...
class Metrics(BaseModel):
    ...
    overall_confidence_intervals: Optional[Dict[str, tuple]] = Field(
        None, description="95% CIs for overall metrics"
    )
    subgroups: List[MetricSubgroup] = Field(
        default=[], description="Metrics broken down by demographic subgroups"
    )
    threshold_used: Optional[float] = Field(
        None, description="Decision threshold for binary classification (e.g., 0.5 probability)"
    )
    notes: Optional[str] = Field(None, description="Additional notes on metrics calculation")
...
class EvaluationData(BaseModel):
    ...
    test_train_split: Optional[Dict[str, float]] = Field(
        None, description="Data split ratios (e.g., {'train': 0.8, 'val': 0.1, 'test': 0.1})"
    )
    stratification: Optional[List[str]] = Field(
        None,
...
class TrainingData(BaseModel):
    ...
    sample_size: Optional[int] = Field(None, description="Total number of training examples")
    ...
    inclusion_criteria: Optional[List[str]] = Field(
        None, description="Inclusion criteria for training data"
    )
    exclusion_criteria: Optional[List[str]] = Field(
        None, description="Exclusion criteria for training data"
    )
    quality_control: Optional[List[str]] = Field(
        None,
        description="QC processes applied (e.g., 'Inter-rater agreement verification (kappa=0.89)', 'Automated artifact detection')",
    )
    data_shifts_addressed: Optional[List[str]] = Field(
        None,
...
class QuantitativeAnalyses(BaseModel):
    ...
    unitary_results: Optional[Dict[str, Any]] = Field(
        None,
        description="Results across single factors (e.g., {'sensitivity_by_age': {...}})",
    )
    intersectional_results: Optional[Dict[str, Any]] = Field(
        None,
...
class EthicalConsiderations(BaseModel):
    ...
    demographic_disparities: Optional[str] = Field(
        None,
    ...
    fairness_considerations: Optional[str] = Field(
        None, description="Discussion of model fairness and equity"
    )
...
class CaveatsRecommendations(BaseModel):
    ...
    known_limitations: List[str] = Field(
        default=[], description="Known failure modes (e.g., 'Sensitivity drops to 87% with extreme heart rates >120 BPM')"
    )
    future_improvements: Optional[str] = Field(
        None,
...
class FDASpecific(BaseModel):
    ...
    classification: Optional[str] = Field(
        None, description="FDA classification (e.g., 'Class II') and regulatory pathway (e.g., '510(k) premarket submission')"
    )
    predicate_devices: Optional[List[str]] = Field(
        None, description="510(k) predicate device K-numbers (e.g., 'K232488', 'K233429')"
    )
    substantial_equivalence_summary: Optional[str] = Field(
        None,
        description="Brief summary of substantial equivalence rationale and predicate comparison",
    )
    drift_monitoring_plan: Optional[str] = Field(
        None,
    ...
    failure_modes: Optional[List[str]] = Field(
        None,
    ...
    human_factors_summary: Optional[str] = Field(
        None,
...
class ModelCard(BaseModel):
    ...
    quantitative_analyses: Optional[QuantitativeAnalyses] = Field(
        None, description="Additional quantitative analyses"
    )
    ethical_considerations: Optional[EthicalConsiderations] = Field(
        None, description="Ethical considerations and potential harms"
    )
    caveats_recommendations: Optional[CaveatsRecommendations] = Field(
        None, description="Caveats, limitations, and recommendations"
    )
    fda_specific: Optional[FDASpecific] = Field(
        None, description="FDA-specific fields for regulatory submissions"
    )
